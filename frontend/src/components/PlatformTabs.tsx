...
export default function PlatformTabs({ platforms, active, onSelect }: Props) {
  const available = PLATFORMS.filter((p) => platforms.includes(p.id));
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                isActive
                  ? 'text-white'
                  : 'text-gray-400 hover:text-gray-200 hover:bg-white/05'
              )}
              style={
                isActive
                  ? {
            ...
            >
              {/* Platform icon */}
              <span style={{ color: isActive ? p.color : '#6b7280' }}>
                {p.icon}
              </span>
              {p.label}
